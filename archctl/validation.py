...
class Validation:
    ...
    def __ask_confirmation(self, msg):
        click.echo(msg)
    ...
    def template(self, t: comm.TemplateVersion):
        """Returns a list of the"""
        self.cli.cw_repo = t.template.template_repo
        templates = [temp.template for temp in util.search_templates(self.cli, t.ref)]
        if t.template.template not in templates:
            if self.interactive:
                return False
            else:
    ...
    def kind(self, repo, kind):
        """Returns a list of the"""
        self.cli.cw_repo = repo
        if kind == "Template" and not util.has_templates(self.cli):
            if self.interactive:
                return False
            else:
        ...
        if yes and cookies is None:
            if self.interactive:
                ...
                raise click.BadParameter(
                    "When running in --yes-all mode, cookies are mandatory"
                )
